  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      function loadMoreResults() {
        let counter = 0; // Counter to keep track of the number of times "More results" is clicked
        const interval = setInterval(() => {
          const clicked = clickMoreResults();
          if (clicked) {
            scrollToBottom();
            counter++; // Increment the counter if "More results" is clicked
            if (counter === 3) { // Stop the loop if "More results" is clicked thrice
...
(async () => {
  const query = "Latest on \"Zilper Trenchless\"";
